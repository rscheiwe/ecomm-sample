...
def authenticate_user(username: str, password: str):
    with db_context() as session:
        user_login = session.query(User).filter(User.email == username).first()

        if user_login:
            if verify_password(password, user_login.__dict__['password_hash']):
                return user_login
        return False


async def get_current_active_user(current_user: user_schemas = Depends(get_current_user)):
...
class RoleChecker:
    ...
    def __call__(self, user = Depends(get_current_user), db: Session = Depends(get_db_session)):
